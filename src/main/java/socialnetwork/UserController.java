...
public class UserController implements Observer {
    ...
    private int pageSizeFriends = 0;
    private int pageSizeCereri = 0;
...
//        nextBttnCereri.setDisable((currentPageCereri + 1) * pageSizeCereri >= totalNrOfElemsRequests);
    ...
    public void handleNextFriend(ActionEvent ev) {
        if((currentPageFriends + 1) * pageSizeFriends < totalNrOfElemsFriends) {
            currentPageFriends++;
            initModelFriends();
        }
    }
    ...
    public void handleNextCerere(ActionEvent ev) {
        if((currentPageCereri + 1) * pageSizeCereri < totalNrOfElemsRequests) {
            currentPageCereri++;
            initModelRequests();
        }
    }
